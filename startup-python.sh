...
# Startup script for Python Backend + Frontend + MCP Server deployment

echo "Starting Prompt House Premium Python Service..."
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if command -v nc >/dev/null 2>&1; then
        if nc -z "${DB_HOST_EXTRACTED}" "${DB_PORT_EXTRACTED}"; then
            ...
            echo "Python backend will handle database connection directly"
            break
        fi
    fi
...
# Start the Python backend server (serves API + Frontend)
echo "Starting Python backend server on port $PORT..."
cd /app/backend-python && python main.py &
BACKEND_PID=$!
